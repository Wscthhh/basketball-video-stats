...
export interface TeamHighlightExport { id: string; matchId: string; teamId: string; status: 'queued' | 'running' | 'completed' | 'failed' | string; progress: number; clipIds: string[]; downloadUrl?: string | null; error?: string; createdAt?: string }
export interface TeamTrainingStatus { threshold: number; ready: boolean; trained: boolean; suggestion: boolean; teams: Array<{ teamId: string; side: string; name: string; sampleCount: number; trainedSampleCount: number; trainedAt?: string | null }> }
