...
$volumeBase = Join-Path $root 'release\CourtTrace-Runtime-0.1.0.7z'
$manifest = Join-Path $root 'release\CourtTrace-Runtime-0.1.0.json'
...
Get-ChildItem (Join-Path $root 'release') -Filter 'CourtTrace-Runtime-0.1.0.7z.*' -ErrorAction SilentlyContinue | Remove-Item -Force
...
@{ version = '0.1.0'; parts = $parts } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $manifest -Encoding ASCII
